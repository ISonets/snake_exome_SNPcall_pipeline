#!/usr/bin/env python3
base = "/mnt/disk1/PROJECTS/EXOMS/DATA/240125_seq/"
# maybe add a variable for pathway? hmmm
suffix_1 = "_R1.fastq"
suffix_2 = "_R2.fastq"
...
want_all.append(expand("res_snake/aln_res/{sample}_aligned_reads.bam",sample=SAMPLES))
...
rule bwa_aln:  # alignment, specifying header for proper GATK
    ...
    threads: 32
    shell: """
           bwa mem -Y -t {threads} -R '@RG\\tID:exome_sample\\tLB:sample_library\\tPL:ILLUMINA\\tSM:exome_sample' {params.hg38} {input.read1} {input.read2} > {output.bwa_aln_res}
    """
rule samtools_bam: # sometimes samtools shits its pants when working w/SAM (smth with headers), BAM conversion solves the issue
    input: bwa_aln_res = "res_snake/aln_res/{sample}_aligned_reads.sam",
    output: bwa_aln_bam = "res_snake/aln_res/{sample}_aligned_reads.bam",
    threads :32
    shell:"""
          samtools view -@ {threads} -b {input.bwa_aln_res} > {output.bwa_aln_bam}
          rm {input.bwa_aln_res}
    """

rule mark_dupl:  # deduplication
    input: bwa_aln_bam = "res_snake/aln_res/{sample}_aligned_reads.bam",
    output: dedup_reads = "res_snake/aln_res/{sample}_dedup_reads.bam",
            dedup_metrics = "res_snake/aln_res/{sample}_dedup_metrics.txt"
    params: hg38 = "/mnt/disk1/PROJECTS/IMMUNE/DATA/hg38/hg38.fa"
    shell: """
           gatk MarkDuplicates -I {input.bwa_aln_bam} -M {output.dedup_metrics}	-O {output.dedup_reads}	--ASSUME_SORT_ORDER queryname -R {params.hg38} --REMOVE_DUPLICATES true
    """

rule aln_idx:  # indexing .bam's, depth calculation, coverage calculation, basic stats
    input: dedup_reads = "res_snake/aln_res/{sample}_dedup_reads.bam",
           snps_coords_0based="/mnt/disk1/PROJECTS/EXOMS/RESULTS/panels/beds/snp_loc_final_0based.bed"
    ...
    shell: """
           ...
           samtools depth {output.dedup_reads_sorted} -a -b {input.snps_coords_0based} -o {output.dedup_reads_depth}
